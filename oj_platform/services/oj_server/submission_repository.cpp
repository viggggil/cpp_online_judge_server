...
std::vector<oj::common::AdminSubmissionListItem>
SubmissionRepository::list_recent_submissions(
    std::size_t limit,
    const std::string& problem_id_filter) const {
    auto connection = mysql_client_.create_connection();

    std::string sql_text =
        "SELECT submission_id, username_snapshot, problem_id_text, language, status, "
        "final_status, accepted, detail, total_time_used_ms, peak_memory_used_kb, "
        "created_at "
        "FROM submissions ";

    const bool has_filter = !problem_id_filter.empty();
    const bool filter_is_numeric = has_filter && [&]() {
        try {
            std::stoll(problem_id_filter);
            return true;
        } catch (...) {
            return false;
        }
    ...
    if (has_filter) {
        sql_text += "WHERE (problem_id_text = ? ";
        if (filter_is_numeric) {
            sql_text += "OR problem_id = ? ";
        }
        sql_text += ") ";
    }

    sql_text += "ORDER BY created_at DESC, id DESC LIMIT ?";

    auto statement = std::unique_ptr<sql::PreparedStatement>{
        connection->prepareStatement(sql_text)};

    int parameter_index = 1;
    if (has_filter) {
        statement->setString(parameter_index++, problem_id_filter);
        if (filter_is_numeric) {
            statement->setInt64(parameter_index++, std::stoll(problem_id_filter));
        }
    }
    statement->setInt(parameter_index, static_cast<int>(limit));

    auto result = std::unique_ptr<sql::ResultSet>{statement->executeQuery()};

    std::vector<oj::common::AdminSubmissionListItem> items;
    while (result->next()) {
        oj::common::AdminSubmissionListItem item;
        item.submission_id = result->getString("submission_id");
        item.username = result->getString("username_snapshot");
        item.problem_id = result->getString("problem_id_text");
        item.language = result->getString("language");
        item.status = result->getString("status");
        item.final_status = result->getString("final_status");
